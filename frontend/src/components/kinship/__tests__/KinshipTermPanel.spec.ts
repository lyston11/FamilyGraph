...
const mountedPanels: Array<{ wrapper: VueWrapper; pinia: Pinia }> = []

afterEach(async () => {
  await flushPromises()
  for (const { wrapper, pinia } of mountedPanels.splice(0)) {
...
async function mountPanel() {
  ...
  mountedPanels.push({ wrapper, pinia })
  await flushPromises()
...
describe('KinshipTermPanel current viewer and optional preference controls', () => {
  ...
  it('keep invalidates all personal term and PFV spaces and reloads the active view', async () => {
    ...
    const position = { x: 321, y: 456 }
    const viewport = { x: -20, y: 35, zoom: 0.7 }
    for (const spaceId of [7, 8]) {
      pfv.positionsBySpace.set(spaceId, new Map([[2, position]]))
      pfv.rememberViewport(spaceId, viewport)
      pfv.rememberViewMode(spaceId, 'canvas')
    }
    ...
    expect(pfvApi.fetchPersonalFamilyView).toHaveBeenCalledWith(7, null, expect.objectContaining({ progressive: true }))
    for (const spaceId of [7, 8]) {
      expect(pfv.positionsBySpace.get(spaceId)?.get(2)).toEqual(position)
      expect(pfv.viewports.get(spaceId)).toEqual(viewport)
      expect(pfv.viewModes.get(spaceId)).toBe('canvas')
    }
  ...
  it('restore uses current projection metadata and leaves other spaces cached', async () => {
    ...
    pfv.bySpace.set(8, snapshot(8))
    const viewport = { x: -20, y: 35, zoom: 0.7 }
    pfv.positionsBySpace.set(7, new Map([[2, { x: 321, y: 456 }]]))
    pfv.rememberViewport(7, viewport)
    pfv.rememberViewMode(7, 'canvas')
    ...
    expect(pfvApi.fetchPersonalFamilyView).toHaveBeenCalledWith(7, null, expect.objectContaining({ progressive: true }))
    expect(pfv.positionsBySpace.get(7)?.get(2)).toEqual({ x: 321, y: 456 })
    expect(pfv.viewports.get(7)).toEqual(viewport)
    expect(pfv.viewModes.get(7)).toBe('canvas')
