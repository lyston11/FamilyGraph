...
async function createLineageSpace(): Promise<void> {
  ...
  try {
    ...
    message.error(error instanceof ApiError ? error.message : '创建族谱空间失败')
  } finally {
...
// ---- 下拉选项 ----
const CAL_OPTIONS: SelectOption[] = [
  { label: '公历', value: 'solar' },
  { label: '农历', value: 'lunar' },
  { label: '不详', value: 'none' },
]
const householdOptions = computed<SelectOption[]>(() =>
  householdSpaces.value.map((s) => ({ label: s.name, value: s.id })),
)
const lineageOptions = computed<SelectOption[]>(() =>
  lineageSpaces.value.map((s) => ({ label: s.name, value: s.id })),
)
</script>
...
<template>
  ...
  >
    <NSteps :current="step + 1" size="small" class="steps">
    ...
    <NForm
      ...
      label-placement="top"
      :show-feedback="false"
      data-test="wizard-step-info"
    >
      <NFormItem label="名字（允许重名）" required>
        <NInput
      ...
      <NFormItem label="与你的关系" required>
        <div class="relation-block">
          <NRadioGroup v-model:value="relationDir" data-test="wizard-relation-dir">
            <NRadio v-for="opt in RELATION_OPTIONS" :key="opt.value" :value="opt.value">
              {{ opt.text }}
            </NRadio>
          </NRadioGroup>
          <NInput
      ...
      <NFormItem label="性别">
        <NRadioGroup v-model:value="form.gender" data-test="wizard-gender">
      ...
      <NFormItem label="出生">
        <div class="date-row">
          <NSelect
            v-model:value="form.birthCalType"
            class="cal-select"
            :options="CAL_OPTIONS"
            data-test="wizard-birth-cal"
          />
          <NDatePicker
            v-if="form.birthCalType !== 'none'"
            :formatted-value="form.birthDate || null"
            type="date"
      ...
      <NFormItem label="去世">
        <div class="date-row">
          <NCheckbox v-model:checked="form.deathEnabled" data-test="wizard-death-enable">
            已去世
          </NCheckbox>
          <template v-if="form.deathEnabled">
            <NSelect
              v-model:value="form.deathCalType"
              class="cal-select"
              :options="CAL_OPTIONS"
              data-test="wizard-death-cal"
            />
            <NDatePicker
              v-if="form.deathCalType !== 'none'"
              :formatted-value="form.deathDate || null"
              type="date"
              value-format="yyyy-MM-dd"
              placeholder="选择日期"
              data-test="wizard-death-date"
              @update:formatted-value="(v: string | null) => (form.deathDate = v ?? '')"
            />
          </template>
        </div>
      </NFormItem>
    ...
    <NForm
      ...
      label-placement="top"
      :show-feedback="false"
      data-test="wizard-step-mode"
    >
      <NFormItem label="这份档案归谁维护？">
        <NRadioGroup v-model:value="form.privacyMode" class="mode-group" data-test="wizard-mode">
          <NRadio value="handover" class="mode-option">
            <span class="mode-body">
              <span class="mode-title">移交本人</span>
              <span class="mode-desc">亲人领取账号后获得全部编辑权，你转为只读。</span>
            </span>
          </NRadio>
          <NRadio value="perpetual" class="mode-option">
            <span class="mode-body">
    ...
    <NForm
      v-else-if="hasSpaces && step === 2"
      label-placement="top"
      :show-feedback="false"
      data-test="wizard-step-space"
    >
      <NFormItem label="加入哪个空间？">
        <NRadioGroup v-model:value="spaceChoice" data-test="wizard-space-choice">
          <NRadio value="none">不加入空间</NRadio>
          <NRadio value="household" :disabled="householdSpaces.length === 0">家庭空间</NRadio>
          <NRadio value="lineage">族谱空间</NRadio>
        </NRadioGroup>
      </NFormItem>
      <template v-if="spaceChoice === 'household'">
        <NFormItem>
          <NSelect
            v-model:value="joinHouseholdSpaceId"
            placeholder="选择家庭空间"
      ...
      <template v-if="spaceChoice === 'lineage'">
        <NFormItem>
          <div class="lineage-block">
            <NSelect
              v-model:value="joinLineageSpaceId"
              placeholder="选择族谱空间"
    ...
    <div v-else data-test="wizard-step-confirm">
      <NDescriptions :column="1" bordered>
        <NDescriptionsItem label="名字">{{ form.name }}</NDescriptionsItem>
        <NDescriptionsItem label="与你的关系">
        ...
        <NDescriptionsItem label="归属模式">
          {{ form.privacyMode === 'handover' ? '移交本人' : '永久管理' }}
        </NDescriptionsItem>
        <NDescriptionsItem v-if="chosenSpaceId" label="加入空间">
