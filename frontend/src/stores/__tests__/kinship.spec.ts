...
describe('kinship store（V2.3 Block E4c）', () => {
  ...
  it('新授权视图失效目标解析时，旧请求不能回写且其他目标缓存保留', async () => {
    const store = useKinshipStore()
    mockedResolve.mockResolvedValue(makeResolve())
    await store.resolvePair(10, 1, 2)
    mockedResolve.mockResolvedValue(makeResolve({ target_user_id: 3 }))
    await store.resolvePair(10, 1, 3)
    const old = deferred<KinshipResolve>()
    mockedResolve.mockReturnValueOnce(old.promise)
    const pending = store.resolvePair(10, 1, 2, { force: true })
    store.invalidateResolve(10, 1, 2)
    expect(store.cachedResolve(10, 1, 2)).toBeNull()
    expect(store.cachedResolve(10, 1, 3)).not.toBeNull()
    old.resolve(makeResolve({ term: '旧输入的称谓' }))
