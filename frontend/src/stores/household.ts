...
import { fetchHouseholdCard } from '@/api/household'
import type { HouseholdCardData, HouseholdCardSnapshot } from '@/types/api'

/**
 * HouseholdCard 状态（design.md §4.2）：按明确 space_id 保存服务端授权家庭卡。
 *
 * 红线：
 * - 只消费 api/household.ts 的已解码投影；不用 `/users`、旧 members 列表或
 *   PersonalFamilyView 路径拼装成员；
 * - 每个请求绑定 space + epoch，切换空间或清理后旧响应一律丢弃（不回写）；
 * - 无乐观更新：304 保留同一快照对象，变更一律重新读服务端。
 */
export const useHouseholdCardStore = defineStore('householdCard', () => {
  const bySpace = ref<Map<number, HouseholdCardSnapshot>>(new Map())
  ...
  function forSpace(spaceId: number): HouseholdCardData | null {
    return bySpace.value.get(spaceId)?.data ?? null
  }

  /**
   * 读取指定空间的 household 投影。`force` 跳过 ETag 条件请求（用户显式刷新）；
   * 否则带上已有 etag，304 时保留同一快照对象。
   */
  async function load(
    spaceId: number,
    options: { force?: boolean } = {},
  ): Promise<HouseholdCardData | null> {
    const requestEpoch = epoch
    const cached = bySpace.value.get(spaceId) ?? null
    setLoading(spaceId, true)
    setError(spaceId, null)
    try {
      const snapshot = await fetchHouseholdCard(
        spaceId,
  ...
  function refresh(spaceId: number): Promise<HouseholdCardData | null> {
