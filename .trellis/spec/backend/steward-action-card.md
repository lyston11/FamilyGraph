...
- **总截止（09-17 B）**：单笔 HTTP 的 `timeout` 是**整笔墙钟上界**（含连接/发送/headers/body/解析），读取循环内按单调时钟强制；不断收到小块数据不得延长总时长，超界即中止并由上下文管理器关闭连接。未发送的预留只释放不标 unknown；已发送且无法确认上游状态保守计费、不自动重发。
