...
- **候选 prompt 契约（09-19 candidate-prompt-contract）**：`_PROMPTS["candidate"]` 必须显式包含三条款，且以 `_CANDIDATE_DIRECTION_CLAUSE` / `_CANDIDATE_CONFLICT_CLAUSE` / `_CANDIDATE_EXAMPLE_CLAUSE` 三常量拼接（回归逐条断言这三段，不做宽泛子串匹配）：①方向语义——`biological_parent`/`adoptive_parent`/`step_parent`/`guardian` 的 `subject` 是 `object` 的父/母/监护人，`spouse`/`partner`/`direct_sibling` 对称；②矛盾/重复禁止——不得输出与 `facts` 矛盾或重复的候选，同一对端点的不同 kind 也属矛盾（显式举 `biological_parent(A,B)` 与 `direct_sibling(A,B)` 互斥）；③至少一个符合 schema 的最小示例，只用 `nNNN` 占位代号。`prompt_version()`（`steward_assist`）是 `_PROMPTS` 的规范化 sha256，评测报告字段必须取它；任何 prompt 变更都会改变版本。prompt 是次要防线，**不替代**输出侧确定性语义拦截。
- **事实规模降级与终态映射（09-19 candidate-prompt-contract）**：`candidate_user_content(db, space_id, ctx)` 是候选 user 内容的唯一构建入口（`schedule_due_batch` 与 `_user_content_for` 共用）。投影字节超预算时按稳定顺序（`fact_id` 升序）取满足「system prompt + 分隔符 + 投影 ≤ `STEWARD_ASSIST_MAX_PROMPT_BYTES`」的最长前缀：子集确定、可复现，`input_hash`/`prompt_digest` 因此稳定，发送前 fence 不误判；不引入 `truncated` 标记（不依赖模型理解元信息），不放宽任何输出校验，不新增硬编码常量。连空投影都装不下（单条事实极长）时返回全量原样，由既有字节上界如实结算。终态映射：批次内存在 `skipped` + `prompt_too_large` 时批次终态 `failed` 且 `error_code="prompt_too_large"`；**不**归入 `unknown`（不伪造上游不确定性）、不计费、不重放。预算类 `skipped`（`budget_exhausted`/`insufficient_budget`）是良性可重试状态，不触发该终态。
- **超限可观测（09-19 candidate-prompt-contract）**：`GET /admin-api/v1/steward/status` 的 `metrics` 含 `assist_skipped`（全部 skipped）与 `assist_skipped_prompt_too_large`（其中确定性超限），使「某空间候选辅助已停止工作」可被发现而不是显示为正常 `applied`。两字段与既有指标同为纯计数，响应仍不含 prompt、事实、姓名或空间内部内容；`test_steward_log_hygiene` 的指标字段白名单必须同步（精确集合断言）。
