...
用户已批准创建最新 C～I 子任务及完整规划；它们全部 planning，本轮不start、不改业务代码、不部署、不真实调用。父 AC-01/04/06/07/08/09 未闭合，保持 in_progress。AC-06 不强制真实提速或降档；要求可靠归因，做优化时再证明对应阶段改善。
...
- 新一轮执行顺序：C → D → E → F → G；H/I 为方案和决策任务，具体映射见任务图。只有最新规划获准后才逐任务start；共享模块/SQLite/端口始终串行。
