...
H/I可以先研究，不阻塞C～G现配置修复。D负责字段和聚合，E负责网关审计/SDK策略，F只补覆盖并把发现退回所属实现；不在三个任务各建一套观测机制。共享backend/agent/SQLite/迁移/端口的一律串行，未来代码在各自start钩子建的worktree中修改。
...
| Q11增量显示方案及安全合同 | H | 方案决策支持AC06；采用后另补实现验收 |
| Q12medium/换模型/并发/轮询/预算取舍 | I | E/H分离；证据与决策支持AC06 |
...
C～G必需修复与验证闭合；H/I给出明确有证据的采用/延期/不采用结论，不能把方案完成写成能力上线。父AC06不强迫降档/换模型，但未解决的归因/程序缺陷不能由产品延期遮盖。AC04/08/09已重新打开，最终报告逐项附证据；符合合并/干净前提后清理父/子worktree与分支。
