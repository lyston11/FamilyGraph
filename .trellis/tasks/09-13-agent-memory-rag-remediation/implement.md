...
- [ ] A：复现 422/500；实现来源与响应合同；补安全重试、并发确认、来源撤销和四开关组合测试。
- [ ] C：把离线压缩复现转回归；预填同一个 manager；补自动触发、重复文本/消息 ID、取消及恢复。
- [ ] B：冻结检索核心集和负例；实现词法计划/短词/分块/预算；测召回；再同步 citations/SSE/历史及 payload 限额。
- [ ] D：添加 tombstone 防复活与唯一性回归；设计增量补建；覆盖重启、RAG-only maintenance、并发/撤销竞争；迁移检查。
- [ ] E：逐项产出采用/延期理由、可执行评估和既有任务交接；MR-23 的完整模型候选→后续 core 路径不能用直接 upsert 代替。
...
- [ ] 集成后执行 frontend-api-smoke；退出码 2 记录环境阻塞，不视为通过。
- [ ] 仅在新变更、失败或未解决疑点时重复检查；审计已有 211 pass 不需为文档创建重跑。
