...
| 子任务 | 状态 | 已产生的可核验材料 |
|---|---|---|
| A | 后端补充回归/核验中 | frontend 独立 check 后 56 tests 通过；真实三 listener smoke 56 项通过（Memory 26）；legacy 恢复越界探针 1 fail→修复后1 pass；后端最终宽检查待回填 |
| C | 等待 A 完成 | 已批准的 SDK 历史恢复与自动/手动压缩回归方案 |
| B | 等待 A/C 完成 | 已冻结中文样本、引用认证和 16 KiB 事件合同 |
| D | 等待 A/B 完成 | 已批准的稳定索引身份、防复活和有界维护方案 |
| E | 隔离复现通过，方案评估中 | MR-23 / MR-26 合成生产链与对照已完成；主线程已核对源码/harness 哈希与关键阶段，12 项决策和预算方案探针整理中 |
