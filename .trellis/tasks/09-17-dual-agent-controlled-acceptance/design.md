...
| 组 | 注入点 | 必需断言 |
| --- | --- | --- |
| A1 无工具/有正文 | SDK首delta与message_end间隔 | 首SDK正文、首安全可见正文、完整正文、持久化、页面可见分别记录（09-18 增量落地后必测） |
...
| UI1 生命周期 | 页面初次打开、提交、完成/失败、刷新 | 真实DOM初帧和过渡，无空泡/重复消息/无限等待；增量落地后验证临时正文不被重复追加 |
