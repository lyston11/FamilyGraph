...
## 8. E-R5 已决策：有界退避、次数不变（2026-09-18）

09-18 用线上 `agent_provider_egress` 审计取得**真实可用性数据**后，用户批准按 A+B 推进，
E 的策略部分据此落地（提交 `4a850d1`，已集成 main `e0ee321`）。
...
| 失败尝试 | 7（5×502 + 2×503） |
| 成功尝试 | 4 |
| **run 级最终结果** | **2/2 succeeded** |

即：**尝试级失败率 64%，但 run 级成功率 100%**。run 2 第二轮恰好**用满 5 次请求层重试**
（6 次尝试）才成功。因此先前候选表里「请求层 5 → 1」会让该轮直接失败——**降次数被真实
数据否决**，不是偏好问题。这也修正了上表把候选写成「降为 1 次」的风险评估不足。

### 选定策略（已实施）

保持**次数不变**（请求层 5 + 会话层 3），改两处**有界**行为：

1. **首响应期限**：`AGENT_PROVIDER_PROXY_HEADER_TIMEOUT_SECONDS`（默认 20s），只约束
   **等待上游响应头**阶段，用 `asyncio.wait_for` 在超时时取消等待。针对 run 1 的「单次 503
   拖 29.8s」：连接超时（10s）与总超时（300s）都拦不住「连接已建立但响应头不来」。
   响应头之后的流式生成不受影响，长回答不会被误杀。超时审计为
   `transport_timeout`/`retryable=true`/`sent=true`。
2. **有界退避提示**：暂时错误返回 `retry-after-ms: 500`（pi-ai 的 `getRetryDelayMs` 优先读
   该头）。500ms 是默认退避 `min(0.5·2^i,8)s` 的**第一级**，故对任意次数都不劣于默认，
   而用满 5 次时从 15.5s 降到 2.5s。针对 run 2 的「5×502 本身仅约 4.5s，退避却加 ~11.6–15.5s」。
   **429 与上游自带 `Retry-After` 不受覆盖**（那会把限流变成每秒捶打）。

### 实测影响（同配置对照）

- 首响应期限：假上游延迟 5s 不返回头时，无该期限会一直等；加入后按期收敛到 502，
  且审计如实标注已发送。
- 有界退避：同一套真实 SDK + 假网关、**相同重试次数**下，指数退避 7297ms → 有界提示 339ms。
- 费用：仍**无法在本仓库给出**（两侧 cost 置零、无价格表）。可确定的是重试次数未变，
  故最坏请求数与费用上限与冻结前一致（24 次），本次只压缩等待、不放大调用数。
