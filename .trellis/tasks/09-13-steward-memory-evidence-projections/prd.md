...
- SP-R5：保留旧版本及收件人的驳回历史；证据记录与重新展示/通知分别处理。拟议首版默认不因换版解除驳回或新增通知，最终用户可见策略在实施前审阅。
...
| SP-AC3 | 无权、已撤销、过期/revision 不符的支撑事实不可采用；不支持归因的类型明确降级 |
...
不实现 Steward shared RAG、TermRegistry 替代、行为水位系统、自动关系确认或平台 admin 开关。共享知识能力仍归 [既有 followups](../09-11-steward-capability-followups/prd.md)，平台开关仍归既有 `09-13-steward-assist-platform-switch-admin`。

本任务不在当前 A→C→B→D 的默认修复执行范围内。实施前需基于最新代码审阅本包及证据归因/用户可见策略，并建立独立分支/worktree。
