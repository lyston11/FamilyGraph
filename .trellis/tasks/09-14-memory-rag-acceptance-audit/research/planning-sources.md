...
- 当前工作流入口：[AGENTS.md](../../../../AGENTS.md)、[Trellis workflow](../../../workflow.md)。用户已经授权创建本任务并记录分析，不重复询问登记权限；本次交付范围为规划，不自动进入B/D修复。
...
- main合并/部署未授权；隔离smoke可以先完成。工作树/分支只有已合并且无未提交业务改动、任务archive后才清理，不用force掩盖缺失前置条件。
