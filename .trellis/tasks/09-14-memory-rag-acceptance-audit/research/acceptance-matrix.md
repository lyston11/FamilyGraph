...
以下表格保留 2026-09-14 基线 bd899b9 的复查判定，不覆盖此前命令输出。续接时 B 已完成修复核验，D 正在执行，C 已进入累计候选；最新版本与待验证范围见 [累计执行检查点](integration-preflight.md)。最终逐项结论在 D 之后回填，不能把以下历史“失败”或 B 的通过外推为当前累计验收结果。
