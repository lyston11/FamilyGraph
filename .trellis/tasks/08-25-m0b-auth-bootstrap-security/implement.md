...
1. [ ] Alembic 迁移：accounts、audit_log（users 已在 m0a 建）
2. [ ] utils/security.py：pin_gen(secrets)/hash(bcrypt 或 argon2)/jwt/challenge —— 单测先行
3. [ ] services/auth_guard.py：失败计数+锁定 —— 分支覆盖测试
4. [ ] api/auth.py：login/select/refresh/logout 全流程 + 统一错误结构
...
# 手工旅程: 首启→管理员登录→建测试账号→错误5次锁定→同名同PIN消歧→改PIN旧token失效
```
