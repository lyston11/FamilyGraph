...
- [x] 确认依赖：`09-11-steward-production-ops`, `09-11-steward-assist-execution`, `09-11-steward-projection-consistency`, `09-11-steward-quality-security`, `09-11-steward-candidate-review`。
- [x] 取得最新实施摘要的明确批准并完成本任务实现；发布门禁仍为 partial。

## 有序执行

- [x] 1. 确认实际 DATA_DIR、迁移 head、三 listener 与新 config 的模板透传；制作不输出秘密的状态检查。
- [x] 2. 实现有界结构化指标和安全异常日志，复用 system-admin-frontend 状态入口。
- [x] 3. 编写隔离 E2E 驱动与故障注入步骤，覆盖真实领域命令和自动 maintenance tick。
- [x] 4. 执行合成规模样本记录瓶颈，仅当数据证明需要才提出独立 worker/增量计算后续。
- [x] 5. 在临时库执行全项目门禁与迁移；若并行工作失败定位到具体文件，不冒称本任务通过全量。
- [x] 6. 输出 release-evidence.md 和 runbook.md（实施时），明确 stub 与真实 provider、已验证与尚缺证据。
