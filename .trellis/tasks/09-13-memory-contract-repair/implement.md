...
- [x] 用当前 UI payload 建真实 API 回归：无来源 422、带来源提交后 500/list 500；保留失败证据。
- [x] 固定 source/legacy/来源依赖 DTO 与迁移；补删除来源、非法 ref、并发确认造数。
- [x] 修 output mapper、提交前 DTO 检验、创建幂等与确认唯一性。
- [x] 接 manual/agent_message/rag_chunk resolver，覆盖每个读写面和来源撤销。
- [x] 修前端来源传递、RAG 原文只读、重复操作 key、Memory/RAG 四组合可达性。
- [x] 将旧服务测试中的伪授权 doc 字符串替换为 manual 或真实授权来源造数，不能为过旧 fixture 放松校验。
- [x] 隔离迁移前后样本检查，记录异常 legacy/重复数据而不删除。
- [x] 向 B/D 交接 source/ref/revision 与权限 helper 合同；记录测试和迁移结果。
...
A-AC1～8 已通过，完整证据见 [research/validation.md](research/validation.md)。后端全套 1050 passed/3 skipped，前端相关 56 passed，真实 listener smoke 56/56。代码已冻结，等待本地提交后串行启动 C；主分支集成不在本轮授权内。
