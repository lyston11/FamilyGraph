...
- [x] 实现 AgentQueryService 与六个版本化只读工具，限制分页/路径长度/输出大小。
- [x] 编写 Assistant 系统提示词，明确事实状态、路径引用、拒绝越权和资料不足行为。
- [x] 实现 Session/Message/Run 用户 API 与端到端只读权限测试。
- [x] 新增 agent API/store/SSE composable；扩展 `auth.clearSession()` 与空间切换清理。
- [x] 在 App.vue 挂全局 Launcher，交付桌面 drawer/移动全屏、Session 列表、消息流、取消/重试。
- [x] 加入工具摘要与证据路径展示，不暴露原始内部 payload。
- [x] 完成 a11y、响应式、断线/刷新/401/撤权/跨 scope 对抗测试。
