"""Independent B contract probes. Synthetic data; no product/test files edited."""
...
def _start_run(db, internal_client, session, query):
...
def _world(db, internal_client, name, summaries=None, query="上海"):
    ...
    memory_ids = [_confirm(db, owner, value) for value in (summaries or ["上海聚餐在周日下午三点。"]) ]
...
def _answer(internal_client, world, payload):
    return internal_client.post(
        ...
        json={"events": [{"seq": 2, "type": "message.assistant_added", "public_payload": payload}]},
...
def _fallback(client, world, headers):
    return client.get(
        f"/api/agent/runs/{world['run_id']}/events/2/citations", headers=headers
    )
...
def test_real_context_obeys_its_documented_utf8_sub_budget(db_session, internal_client):
    ...
    estimated = memory_rag._estimate_tokens(json.dumps(blocks, ensure_ascii=False))
    print("B_BUDGET", {"blocks": len(blocks), "stored_budget": build.token_budget, "wrapped_utf8_estimate": estimated})
...
def test_citation_requires_exact_original_chunk(db_session, internal_client, client, mutation):
    ...
    db_session.commit()
    appended = _answer(internal_client, world, {"role": "assistant", "text": f"合成回答 [{block['citation']}]"})
...
def test_repeated_context_invalidates_when_rag_is_disabled(db_session, internal_client):
    ...
    print("B_RAG_DISABLED", {"status": second.status_code, "blocks": len(second.json().get("context_blocks", []))})
...
def test_internal_history_drops_revoked_citation_identifiers(db_session, internal_client, client):
    ...
    assert _answer(internal_client, world, {"role": "assistant", "text": f"合成回答 [{handle}]"}).status_code == 200
    settled = internal_client.post(
        f"/internal/agent/runs/{world['run_id']}/settle",
        json={"status": "succeeded"}, headers=world["headers"],
    )
    assert settled.status_code == 200, settled.text
    memory_rag.revoke_memory(db_session, memory_id=world["memory_ids"][0], account_id=world["owner"].account.id)
    db_session.commit()
    public_history = client.get(
        f"/api/agent/sessions/{world['session'].id}/messages", headers=_public_headers(client, world)
    ).json()
    public_assistant = next(item for item in public_history if item["role"] == "assistant")
    assert public_assistant["citations"] == [] and public_assistant["unavailable_citation_count"] == 1
    ...
    print("B_INTERNAL_HISTORY", {"public_unavailable": public_assistant["unavailable_citation_count"], "internal_stored_citations": historical["content_json"].get("citations")})
    assert historical["content_json"].get("citations", []) == []


def test_sse_does_not_trust_citations_supplied_in_public_payload(db_session, internal_client, client):
    world = _world(db_session, internal_client, "b-sse-forged")
    fake = {"source_type": "memory", "source_id": "not-a-source", "scope": "private", "sensitivity": "normal", "revision": 99, "citation_handle": "forged-handle", "text": "synthetic unverified excerpt"}
    appended = _answer(internal_client, world, {"role": "assistant", "text": "普通回答", "citations": [fake]})
    assert appended.status_code == 200, appended.text
    settled = internal_client.post(
        f"/internal/agent/runs/{world['run_id']}/settle", json={"status": "succeeded"}, headers=world["headers"]
    )
    assert settled.status_code == 200, settled.text
    headers = _public_headers(client, world)
    sse = client.get(f"/api/agent/runs/{world['run_id']}/events", headers=headers)
    assert sse.status_code == 200
    events = [json.loads(line.removeprefix("data: ")) for line in sse.text.splitlines() if line.startswith("data: ")]
...
def test_fallback_is_scoped_to_the_run_session_and_assistant_role(db_session, internal_client, client):
    other_owner, other_space = create_agent_fixture(db_session, name="b-shadow-earlier")
    other = create_agent_session(db_session, account_id=other_owner.account.id, space_id=other_space.id)
    ...
    assert _answer(internal_client, world, {"role": "assistant", "text": f"合成回答 [{handle}]"}).status_code == 200
    headers = _public_headers(client, world)
    history = client.get(f"/api/agent/sessions/{world['session'].id}/messages", headers=headers).json()
    assistant = next(item for item in history if item["role"] == "assistant")
    assert len(assistant["citations"]) == 1
    fallback = _fallback(client, world, headers)
    assert fallback.status_code == 200, fallback.text
    print("B_FALLBACK_SCOPE", {"history_count": len(assistant["citations"]), "fallback": fallback.json()})
...
def _replace_lease(run_id, *, running=False):
    ...
    with SessionLocal() as other:
        ...
        grant = agent_queue.lease_next(other, kind="assistant", leased_by="replacement", ttl_seconds=60)
        assert grant is not None and grant.run.id == run_id and grant.job.attempt == 2
        if running:
            agent_events.append_events(other, grant.run, [agent_events.EventEntry(seq=2, type="run.started", public_payload={})])
            other.commit()


@pytest.mark.parametrize("endpoint", ["context", "events", "heartbeat", "settle", "tool"])
def test_signed_attempt_is_fenced_after_authorization(db_session, internal_client, monkeypatch, endpoint):
    ...
    if endpoint == "context":
        ...
        response = _answer(internal_client, world, {"role": "assistant", "text": "stale attempt wrote this"})
    elif endpoint == "heartbeat":
        response = internal_client.post(f"/internal/agent/jobs/{world['job_id']}/heartbeat", json={"lease_ttl_seconds": 3600}, headers=world["headers"])
    elif endpoint == "settle":
        response = internal_client.post(f"/internal/agent/runs/{run_id}/settle", json={"status": "succeeded"}, headers=world["headers"])
    else:
        response = internal_client.post(f"/internal/agent/runs/{run_id}/tools/familygraph.echo/execute", json={"version": 1, "input": {"text": "stale tool"}, "tool_call_id": "b-stale-tool"}, headers=world["headers"])
    assert switched
    with SessionLocal() as inspect_db:
        ...
        stored_answer = inspect_db.scalar(select(AgentRunEvent).where(AgentRunEvent.run_id == run_id, AgentRunEvent.type == "message.assistant_added"))
        observed = {"endpoint": endpoint, "http": response.status_code, "current_attempt": current.attempt, "current_status": current.status, "stale_answer_persisted": stored_answer is not None, "tool_result_persisted": stored_tool is not None}
    print("B_STALE_ATTEMPT", observed)
    assert response.status_code in (403, 409), observed


def test_source_revocation_retry_keeps_original_committed_event(db_session, internal_client, client):
    ...
    row = db_session.scalar(select(AgentRunEvent).where(AgentRunEvent.run_id == world["run_id"], AgentRunEvent.seq == 2))
    ...
    assert (row.id, row.public_payload, row.request_fingerprint, row.context_reference_json) == original
    fallback = _fallback(client, world, _public_headers(client, world))
    assert fallback.status_code == 200 and fallback.json()["citations"] == [] and fallback.json()["unavailable_citation_count"] == 1
    print("B_RETRY_CONTROL", {"duplicates": retried.json()["duplicates"], "fallback_unavailable": 1})


def test_concurrent_same_attempt_context_returns_one_reused_build(db_session, internal_client, monkeypatch):
    ...
    def synchronized_scalar(self, statement, *args, **kwargs):
        ...
        if result is None and any(item.get("entity") is ContextBuild for item in entities):
            barrier.wait()
        return result
    ...
    builds = db_session.scalars(select(ContextBuild).where(ContextBuild.run_id == world["run_id"])).all()
...
def test_fts_refills_after_source_dependency_denies_top_hit(db_session, internal_client):
    world = _world(db_session, internal_client, "b-fts-refill", summaries=["needlexyz shared note"], query="needlexyz")
    root_document = db_session.scalar(select(RAGDocument).where(RAGDocument.source_type == "memory", RAGDocument.source_id == str(world["memory_ids"][0])))
    ...
    candidate = memory_rag.propose_candidate(
        ...
        source={"kind": "rag_chunk", "document_id": root_document.id, "chunk_id": root_chunk.id, "revision": root_document.revision, "index_version": root_chunk.index_version, "space_id": world["space"].id},
        source_quote=root_chunk.text,
        summary="needlexyz shared note",
        suggested_scope="private",
        purpose="synthetic dependent source",
    )
    copied = memory_rag.confirm_candidate(db_session, candidate_id=candidate.id, confirmer=world["owner"], confirmer_account=world["owner"].account, scope="private")
    db_session.commit()
    valid_id = _confirm(db_session, world["owner"], "needlexyz shared note")
    memory_rag.revoke_memory(db_session, memory_id=world["memory_ids"][0], account_id=world["owner"].account.id)
    db_session.commit()
    common = dict(actor=world["owner"], account=world["owner"].account, space_id=world["space"].id, query="needlexyz", agent_kind="assistant", for_model=False)
    ...
    print("B_FTS_REFILL", {"denied_copy_id": str(copied.id), "valid_id": str(valid_id), "limit1": limited_ids, "limit20": control_ids})
    assert str(valid_id) in control_ids
    assert limited_ids == [str(valid_id)]


def test_exact_16k_public_payload_preserves_text_web_and_citation_fallback(db_session, internal_client, client):
    world = _world(db_session, internal_client, "b-exact-byte-control")
    context = _context(internal_client, world)
    assert context.status_code == 200, context.text
    handle = context.json()["context_blocks"][0]["citation"]
    payload = {"role": "assistant", "text": f"[{handle}]\n" + '界🙂"' * 1000, "web_citations": [{"url": "https://example.com/synthetic", "title": "合成引用", "excerpt": "合成摘要", "trust": "external"}]}
    ...
    assert appended.status_code == 200, appended.text
    event = db_session.scalar(select(AgentRunEvent).where(AgentRunEvent.run_id == world["run_id"], AgentRunEvent.seq == 2))
    ...
    rejected = internal_client.post(f"/internal/agent/runs/{world['run_id']}/events/append", headers=world["headers"], json={"events": [{"seq": 3, "type": "message.assistant_added", "public_payload": oversized}]})
    assert rejected.status_code == 422, rejected.text
    print("B_16K_CONTROL", {"accepted_utf8_bytes": 16384, "web_preserved": True, "fallback_citations": 1, "oversize_status": rejected.status_code})
