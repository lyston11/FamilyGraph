...
2026-09-14 用户进一步授权“直接开始审查代码并验收，没问题就提交合并归档”。下面波次进入执行；B、D 各在原任务 worktree 串行修复，最终由唯一集成通道验收、提交、合并 main、归档和清理。部署与线上数据操作不在范围内。累计候选已纳入 C `8e91c42` 和 main `d7629df`，原审计证据保持不变。
...
2026-09-14：波次0～7已完成实施与验收，原20组缺口及追加恢复回归闭合。a83b5d1已合入main并push，实际main隔离smoke95/95；七个完成任务均已归档并清理worktree/本地分支。完整结果见 [最终验收](research/final-acceptance.md)；下面保留执行顺序及原放行条件。

从主检出读取实际 task 状态/最新分支，按既有工作流激活对应 B/D 任务，在其专属 worktree 提交。B 已接入累计审查材料与 C 补丁；B 完成后 D 继承该候选。本任务负责最终复验材料，不复制竞争实现所有者。
...
- 通过后由本会话作为单一串行集成通道合并、archive；仅在 worktree 无未提交业务改动且分支已合并时删除 worktree/分支，报告结果。未来 P2 规划不误报实施或归档。
