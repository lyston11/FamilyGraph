...
- [ ] 1. 先写扫描相同 cursor、失败恢复、旧 lease fence 的失败回归；记录现有 46 条测试基线。
- [ ] 2. 补 schedule/job 元数据与迁移，调度器复用 canonical enqueue 合同，禁止直接更新 SourceFact。
- [ ] 3. 实现恢复、退避、固定执行水位、租约栅栏和三 listener 生命周期。
- [ ] 4. 实现 admin 元数据/重跑命令及最小后台运维入口；审计只保存安全理由分类和关联 ID。
- [ ] 5. 更新 Compose/README 中有效开关说明；遵守 scripts/dev-up.sh 启动约定，不改用户 .env。
- [ ] 6. 运行本任务测试；将外部模型慢请求不阻塞调度的集成检查交给 assist-execution 联合验收。
