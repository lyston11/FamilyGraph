...
| Q11 | delta等到message_end发布的边界已证实；增量方案须解决重放/安全/引用/取消/兼容。**H 已归档，由 09-18 承接并进入实施**。 | 09-18 |
...
C 总截止 → D 计时/聚合与关联合同 → E 错误分类/完整审计/重试 → F 受控全链矩阵 → G 部署与真实验证。共享backend/SDK/SQLite/端口时一律串行。I 可先写方案，收益/采用判断依赖D/F/G证据，不阻塞现配置可靠性修复；09-18（含原 H 增量合同）按体验目标实施，P0-1 已部署。D定义请求元数据，E生产审计，F验其一致；避免多处各造一套协议。
...
仍未闭合：Q01/Q02 已在 C 完成（待集成复验）；Q08 属 F；Q09/Q10 属 G；Q11 属 09-18（原 H 已归档）；Q12 属 I；E-R3/E-R5 的预算数值选择待用户批准。

C～G是必需闭合项；09-18 承载体验目标与增量交付（原 H 已归档）；I 是决策交付，创建并不批准上线/调参。G真实小样本上限是提案（见G PRD），须单独确认费用等，不自动消耗。
