...
| 管家调用结果保全与批次时限 | `e482f59`、`9c4a677`；逐笔结算（返回即在其执行身份仍有效时提交，再发下一笔）、混合批次独立产物可消费（批次仍如实 `failed`）、恢复器持久产物优先、单笔 HTTP 整笔墙钟总截止。unknown 仍保守计费且不重放。见[验收记录](tasks/archive/2026-09/09-17-steward-attempt-result-integrity/research/evidence/result-integrity-2026-09-17.md) |
| 助手响应延迟分段定位 | `076d631`、`32b80f1`；`GET /admin-api/v1/agent/latency` 新增 `assistant_phases`，从既有 `agent_run_events.created_at` 拆出 `queue_wait`/`first_text`/`model_turn`/`tool_call`/`settle`，**无需新增字段**；真实只读样本（n=2）显示 `model_turn` 逐轮 p50 33.15s 为主导，排队/工具/落库均非主导。delta→可见差值已用真实 SDK 实测为发布时机（`message_end` 才发布）。**未做提速改动**，逐字显示/换模型/扩容均待用户决定。见[验收记录](tasks/archive/2026-09/09-17-assistant-latency-diagnosis/research/acceptance-2026-09-17.md) |
| 双 Agent 延迟集成验收 | 子任务 B/A 已归档并清理；集成基线 `main@651ee07`。集成期补测两项助手事实：**① `model_turn` 含上游重试退避**（A-02），故新增 `provider_retry`（下界）/`provider_failed_attempts`（无歧义）/`runs_with_provider_retry`——run 2 的 5 次 502 在同一轮内，该轮 33.15s 中 10.22s 是重试；**② 助手实际推理档位是 SDK 默认 `medium`**（平台无档位控制项）。**父任务不归档**：AC-01/06/07 的真实提速与受控场景矩阵未完成（R8）。见[集成验收](tasks/09-17-dual-agent-latency-result-integrity/research/integration-acceptance.md) |
