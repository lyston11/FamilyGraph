...
- **2026-09-20 线上环境上线：`https://fg.lyston.qzz.io`（与开发环境完全隔离）**。服务器上新增第二套容器化栈（独立 clone `/home/ubuntu/fg-prod`、卷 `familygraph-prod_app_data`、全新 `SECRET_KEY`/`AGENT_SERVICE_SECRET`/`ADMIN_JWT_SECRET`、宿主回环 8100/8101），经现有 Cloudflare 隧道的 `fg.lyston.qzz.io` Public Hostname 直连 `127.0.0.1:8100`（**不经宿主 nginx**，宿主 nginx 与其他站点未改动）。系统管理员后台**不挂任何公网 hostname**，仅 `ssh -L 8101:127.0.0.1:8101 lyston` 进入；线上 api/agent 不发布宿主端口。演示账号（朱元璋等 51 人，PIN 123456）由线上 `DEV_SEED_DEMO_DATA=1` 独立播种，首登不改密。验收全绿：公网 health 200、朱元璋公网登录 200、跨环境 token 双向 401、停线上不影响开发、备份 `integrity_check=ok`、安装器幂等。部署入口 `deploy/production/`，合同见 [spec/architecture/13](spec/architecture/13--13-production-deployment-2026-09-20.md)，逐条证据见 [verification.md](tasks/archive/2026-09/09-20-production-deployment-isolated/verification.md)。
  ...
  - **发布用一条命令：`cd ~/fg-prod && bash scripts/deploy-prod.sh`**（默认发 `origin/main`，也可指定 sha）。它先备份再发布，并在失败时按**迁移安全闸门**决策：库迁移未前进则自动回滚代码；**已前进则拒绝回滚**并停下要求人工决策（旧代码配新 schema 比新版本起不来更难诊断）；从不执行 `alembic downgrade`。逐条验收与实现期修掉的 4 个缺陷见 [verification.md](tasks/archive/2026-09/09-20-prod-release-script/verification.md)。
...
- **2026-09-18 助手响应延迟根因分析完成**：完成 FamilyGraph agent（10-30s 空白）vs Pi agent（1-2s 体感）的对比调查。**主因是流式可见性链路中断**：`agent/src/events.ts` 显式丢弃 SDK 的 `message_update`/`text_delta` 事件，用户在整条消息生成期间（实测中位数 11.47s）只能看到空白等待。Pi 的「1-2 秒」体感来自**首 token 或 thinking/正文开始滚动**，而非完整回答结束（对本地 Pi 约 1.2 万条消息统计：同一 `liu-dada/gpt-5.6-sol` 模型的完整生成时长中位数 = 11.47 秒，低于 3 秒的比例仅 0.8%）。次级因素包括：请求层重试放大上游不稳定（实测尾部 11.6-15.5s 全是退避，Pi CLI 请求层 0 次重试）、排队轮询 2s（b067079 已改 250ms 但远端未部署）、cache key 不稳定（每 run 随机 sessionId → cache miss，Pi 有 90%+ cache hit）、per-chunk DB 事务（~200-500ms）、httpx client 每次 TLS 握手（~100-500ms）。优化策略已明确 P0/P1/P2 分级，**P0-1（部署轮询修复）成本为零**，**P0-2（安全增量显示）为体感收益最大项**。详见 [Pi vs FamilyGraph 延迟对比分析](tasks/archive/2026-09/09-18-assistant-low-latency/research/pi-vs-familygraph-latency.md) 与 [执行计划](tasks/archive/2026-09/09-18-assistant-low-latency/implement.md)。**P0-1 已实施并部署**：b067079/4744549 已并入 main（d0ad77d）并推送，远端 backend 与 agent sidecar 均已同步代码、重建 dist 并重启（2026-09-18 08:12 UTC 后运行新版本）。**P0-2 安全增量显示已实现并部署**（`08e5aab` → main `f528ab0`/`9377e98`；远端 2026-09-18 09:24 UTC 重启 backend + sidecar 并重建 dist）：新增 `assistant.text_delta`/`assistant.text_reset` 两个事件类型；sidecar `RunEventBuffer` 有界聚合（单帧 2000 码点，非 delta 事件处 flush，一个 token 不再写一行 DB）；后端闭合形状校验且**不物化 `AgentMessage`**（机制上保证临时正文不进历史/Memory/RAG）；前端累积到 `provisional` 气泡、权威消息到达时整体替换、reset 时立即隐藏。**已核实的关键限制：当前 append 路径与 `message_end` 都没有输出侧正文扫描**，因此增量分片与完整消息面对同一（缺失）检查——实现按合同采用「明示未覆盖」分支，不声称前缀检查等价，也不把最终覆盖当作对已显示片段的撤回。**LL-AC4 已于 2026-09-18 完成**：真实浏览器证明「完整答案前看到安全正文并持续递进、分片拼接与权威文本逐字节一致、刷新重放无残留临时气泡且不重复不串主体、上游失败不被伪装成功」。同一批 n=17 短问答实测：`first_text_ms` 中位数 1977ms、`duration_ms` 中位数 2618ms，**「已生成但不可见」从整条生成时长降到 151–491ms**；完整答案 17/17 ≤ 8s，首段 16/17 ≤ 3s（唯一超标的 run 10 是上游自身 `header_ms=7718`）——**不宣布「3s 必达」**。**P1-1 cache key 收益保持未实测**（重复 prompt 无「首次高后续低」单调模式，且仓库无 usage/cacheRetention 落库字段可核验）。**P2-1（per-chunk DB 事务）经实测否决**：`_refresh_run_gate` 只有 median 368µs/p95 409µs，删掉它会破坏流中取消立即停止转发的合同；**P2-2（复用 httpx client）实测仅省 ≈49.5ms/请求**，留档待另立。证据：`tasks/archive/2026-09/09-18-assistant-low-latency/research/acceptance-2026-09-18.md`、`browser-acceptance-2026-09-18.md`。

- **2026-09-17 双 Agent 最新复核**：原 A/B 已归档，但 `d8d3668` 的阻塞 I/O 总截止、助手源计时/重试统计仍有反例；只读线上核查显示代码同步后服务未加载修复，浏览器未验。父任务保持 in_progress，新增 C～I 七个子任务，见[最新结论](tasks/09-17-dual-agent-latency-result-integrity/research/review-summary.md)和[任务图/全部PRD入口](tasks/09-17-dual-agent-latency-result-integrity/research/remediation-task-map.md)。**C（管家可中断总截止）与 D（助手源计时与重试统计）已在各自分支实现并验证，已串行集成**；**E（网关错误分类与分层重试治理）已集成 main**——上游永久 4xx 不再折叠为可重试 502（实测最坏 24 次出站降为 1 次），每次出站尝试留恰好一条安全审计，两层预算显式冻结，并新增首响应期限与瞬态退避上限；**降低总重试预算仍未批准**（策略表见 E 证据，待用户选择）。**F 已完成并归档**（受控矩阵 41/41 + 真实浏览器 22/22，零真实模型费用；发现并闭合 D-F1 取消分类/D-F2 压缩归属/D-F3 终态标记三个缺陷，另发现并修复「撤权后 Run 不收敛」P1）；**G 已完成并归档**（发布准入→部署→有界真实小样本 4/4，物理请求闸门在出站边界硬封顶，费用上界约 $0.61）；**I 已完成并归档**（零费用排队/吞吐探针 + 决策台账，建议维持当前配置；实测**账户并发上限=2** 是扩容硬约束）；**H 已归档**（方案任务未实施，增量协议与安全合同由 09-18 承接）；**09-18 助手低延迟**：P0-1/P0-2/P1-1 均已实现并部署，**LL-AC4 真实浏览器验收已通过**；P2-1 实测否决、P2-2 留档待另立。**父任务 AC-01～09 已全部闭合**，逐项判定与未闭合项见[总验收报告](tasks/09-17-dual-agent-latency-result-integrity/research/final-acceptance-2026-09-19.md)。下方旧成果表按原时点理解。
