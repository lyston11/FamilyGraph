...
- 演示数据集「明皇室 + 九户外戚世家」：51 名可登录成员（PIN 统一 123456，公开
  ...
  吕府、朱高炽进张家、朱瞻基进孙家、朱祁镇进钱家、朱佛女进李家、宁国公主进
...
_SEED_SPACE_MEMBERS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    # 李家 household 的帝室 member 是朱佛女本人（李贞之妻）——两人互为配偶，
    # 家庭卡上能显示真实称谓；早前误放的 朱元璋 在此空间内没有任何结构事实
    # （他与朱佛女是 sibling，而 sibling 不产生 Relation），会渲染成孤立节点。
    # 朱元璋也不在「李氏家族」lineage：要看李氏家族树必须另行提交 lineage 申请，
    # 并由李贞本人批准（seed 不代替审批）。
    "李家": (
        ("李贞", "m"),
        ("朱佛女", "f"),
