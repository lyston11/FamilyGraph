...
- env=1 + 空库 → 全量播种：12 用户（含出生日期）/3 空间（household×2 + lineage）/
  19 成员行/14 关系/14 confirmed SourceFact（全局一份）/基础五类披露全局开放，
  PIN 123456 校验通过；进程内重复调用（_SEED_DONE）幂等跳过；
...
def test_seed_creates_demo_family_on_empty_db(db_session, monkeypatch) -> None:
    ...
    assert second_space.kind == "household"
    ...
    assert db_session.query(SpaceMember).count() == 19
    ...
    assert len(spouse_edges) == 4 and len(elder_edges) == 10
    ...
    assert len(facts) == 14  # 全局事实（space_id=NULL），三空间共享投影
    ...
    assert len(prefs) == 12 * 5  # 12 用户 × 基础五类
    ...
    assert dev_seed.maybe_seed_demo_data(db_session) is False
    assert db_session.query(User).count() == 12
    # 幂等（路径二）：清掉单例后再跑 → 清单已完全收敛，零写入返回 False
    dev_seed._SEED_DONE = False
    assert dev_seed.maybe_seed_demo_data(db_session) is False
    assert db_session.query(User).count() == 12
    assert db_session.query(SourceFact).count() == 14
...
def test_non_empty_db_backfills_manifest_and_preserves_existing_user(
    ...
    # 固定清单全部补齐：12 演示用户 / 3 空间 / 19 成员行 / 14 关系 / 14 fact
    ...
    assert db_session.query(FamilySpace).count() == 3
    assert db_session.query(SpaceMember).count() == 19
    assert db_session.query(Relation).count() == 14
    assert db_session.query(SourceFact).count() == 14
...
def test_manifest_extension_backfills_missing_rows_only(db_session, monkeypatch) -> None:
    ...
    # 只增：13 用户 / 20 成员行 / 15 关系 / 15 fact
    assert db_session.query(User).count() == 13
    assert db_session.query(SpaceMember).count() == 20
    assert db_session.query(Relation).count() == 15
    assert db_session.query(SourceFact).count() == 15
    # 既有 12 用户行 id 与关键字段零变动
    for user in db_session.query(User).all():
        if user.name == "王二丫":
            continue
    ...
    # 披露偏好只补新成员的 5 行（13×5），既有用户披露不动
    assert db_session.query(DisclosurePreference).count() == 13 * 5
...
def test_converged_manifest_returns_false_and_writes_nothing(db_session, monkeypatch) -> None:
    ...
    assert db_session.query(Relation).count() == 14
    assert db_session.query(SourceFact).count() == 14
    assert db_session.query(DisclosurePreference).count() == 12 * 5
