...
def test_real_tick_rolls_back_entire_rag_batch_and_preserves_core(
    ...
    # The production tick dispatches core computation without waiting for it.
    # Wait only in this assertion path; RAG rejection must leave the job free
    # to finish in its own transaction after the failed maintenance batch.
    with steward_runtime._lock:
        futures = list(steward_runtime._active.values())
