...
def _ctx(actor):
    return ActorContext(
        user_id=actor.id, account_id=actor.account.id, account_status=actor.account.status
    )
...
def test_merge_requires_same_space_and_repoints_relation_creator(db_session):
    ...
    birth = {"cal_type": "solar", "date": "1948-03-12"}
    survivor = create_user_with_pin(
        db_session, "李秀英", "222222", claim_status="managed", birth=birth, created_by=actor.id
    )
    retired = create_user_with_pin(
        db_session, "李秀英", "333333", claim_status="managed", birth=birth, created_by=actor.id
    )
    space = _space(db_session, actor, survivor, retired)
    relation = Relation(
        from_user=survivor.id,
        to_user=actor.id,
        dir_class="elder",
        created_by=retired.id,
        status="pending",
        created_at=utcnow(),
        updated_at=utcnow(),
    )
...
def test_merge_rejects_pair_outside_requested_space(db_session):
    ...
    birth = {"cal_type": "solar", "date": "1948-03-12"}
    survivor = create_user_with_pin(
        db_session, "王丽", "222222", claim_status="managed", birth=birth, created_by=actor.id
    )
    retired = create_user_with_pin(
        db_session, "王丽", "333333", claim_status="managed", birth=birth, created_by=actor.id
    )
    space = _space(db_session, actor)
    db_session.commit()
    try:
        member_commands.merge_duplicate_profile(
            db_session,
            _ctx(actor),
            space_id=space.id,
            survivor_id=survivor.id,
...
def test_merge_does_not_repoint_revoked_facts(db_session):
    ...
    birth = {"cal_type": "solar", "date": "1948-03-12"}
    survivor = create_user_with_pin(
        db_session, "赵强", "222222", claim_status="managed", birth=birth, created_by=actor.id
    )
    retired = create_user_with_pin(
        db_session, "赵强", "333333", claim_status="managed", birth=birth, created_by=actor.id
    )
    space = _space(db_session, actor, survivor, retired)
    fact = SourceFact(
        fact_type="direct_sibling",
        subject_user_id=retired.id,
        object_user_id=actor.id,
        space_id=space.id,
    ...
    reloaded = db_session.get(SourceFact, fact.id)
    assert reloaded is None or reloaded.state == "revoked"
