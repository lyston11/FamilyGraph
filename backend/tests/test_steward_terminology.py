...
def test_deterministic_scan_publishes_baseline_without_empty_projection_or_notifications(
    db_session,
) -> None:
    ...
    edge = next(item for item in payload["edges"] if item["to_user_id"] == gm.id)
    assert edge["term"] == "外婆"
    assert edge["term_source_level"] == "locale"
