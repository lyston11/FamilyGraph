...
1. **移动文件**：
   ```bash
   git mv .agent-notes/implemented/architecture/2026-08-18-xxx.md .agent-notes/archived/architecture/2026-08-18-xxx.md
   ```
2. **插入归档日期标记**：
...
4. **计算哈希并封印入 `manifest.json`（可选进阶）**：
   在 `.agent-notes/archived/manifest.json` 中记录该归档文件的 SHA-256 哈希值。一旦封印，后续任何对归档内容的篡改都会在门禁中报警。
