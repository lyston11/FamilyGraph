...
- **代码入口反向绑定**：每篇落地 Note 必须在核心代码入口留一行反向注释（`// Note: ... 见 .agent-notes/...`），为下一个接手的 Agent 建立物理索引。
...
2. 没有现成的决定记录 → 建最小笔记树：轻量项目只建 `.agent-notes/implemented/<class>/`，用到 `proposed` / `rejected` / `archived` 再建；空目录可直接删。
...
- 实现点在核心入口（优先选公开接口/类型定义、模块顶层导出或核心状态机入口）留一行注释指回这篇：`// Note: 用 SQLite 存会话，为何不用 JSONL — 见 .agent-notes/implemented/feature/2026-08-23-xxx.md`，不逐行标。决定被取代时，这些注释就是要同步的代码清单。
...
```sh
node --experimental-strip-types scripts/verify-agent-note-tree.ts
node --experimental-strip-types scripts/verify-agent-note-format.ts
node --experimental-strip-types scripts/archive-agent-note.ts <note-path>

# 日常开发（轻量直连，推荐）：在工作目录快速生成轻量看板 board.html (仅 ~69KB)
node --experimental-strip-types scripts/build-board.ts --init [targetPath] [projectName]

# 静态打包（脱机大单体）：生成包含全量内嵌笔记数据的单文件 demo.html (供脱机分发或 GitHub Pages)
node --experimental-strip-types scripts/build-board.ts --bundle [notesDir] [outputPath] [projectName]
```

### 💡 工程决策看板运行机制 (Agent Notes Board)

**1. 日常开发模式（推荐）：**
- 运行 `node --experimental-strip-types <skill路径>/scripts/build-board.ts --init board.html "项目决策看板"` 生成约 69KB 的纯模板文件；
- 用浏览器打开后，点击右上角 **「连接本地目录」** 授权选择 `.agent-notes`，浏览器通过 File System Access API 直读本地；
- **零构建、零大单体冗余**：开发或 Agent 增删改 Note 后，只要切回浏览器窗口立刻自动静默热更新！

**2. 离线演示与静态分发模式：**
- 运行 `node --experimental-strip-types <skill路径>/scripts/build-board.ts --bundle .agent-notes demo.html "项目决策看板"`；
