...
def record(
    ...
    suite.add(
        Result(
            ...
            note=note
            if ok
            else f"{note}; expected status={expect_status} code={expect_code}",
            passed=ok,
        )
    )
...
def run_memory_cases(
    ...
    if not check(
        ...
        "/memory-candidates",
        "explicit source and raw_quote projection",
    ):
        return
    response = request(
        "create-retry", "POST", "/memory-candidates", 201, payload=payload
    )
    check(
        "create-idempotent",
        response.status_code == 201 and response.json().get("id") == candidate_id,
        "/memory-candidates",
        "same request returns original candidate",
    )
    request(
        "key-conflict",
        "POST",
    ...
    check(
        ...
        "/memory-candidates",
        "candidate list serializes the committed row",
    )
    response = request(
        "pending-not-indexed",
        "GET",
    ...
    check(
        ...
        "/rag/search",
        "unconfirmed candidate is not searchable",
    )
    ...
    response = request(
        ...
        confirm_path,
        200,
        route="/memory-candidates/{id}/confirm",
    ...
    if not check(
        "confirmed-contract",
        isinstance(memory_id, int) and memory.get("raw_quote") == original_text,
        "/memory-candidates/{id}/confirm",
        "confirmation returns a readable memory",
    ):
        return
    response = request(
        "confirm-retry",
    ...
    check(
        ...
        "/memory-candidates/{id}/confirm",
        "retry preserves memory and retention",
    )
    response = request(
        "search-confirmed",
        "GET",
    ...
    if not check(
        ...
        hit is not None,
        "/rag/search",
        "confirmed memory supplies a versioned source",
    ):
        return
    ...
    response = request(
        "rag-confirm",
        "POST",
        f"/memory-candidates/{copy_candidate_id}/confirm",
        200,
        route="/memory-candidates/{id}/confirm",
    ...
    check(
        "source-revocation-redacts-copy",
        copied is not None
        and copied.get("source_status") == "unavailable"
        and copied.get("raw_quote") is None
        and copied.get("content") is None,
        "/memories",
        "owner metadata remains while source-dependent text is hidden",
    )
    for case_id, query in (
        ("original-no-longer-indexed", "amber box"),
        ...
        check(
            ...
            response.status_code == 200 and response.json() == [],
            "/rag/search",
            "revoked source and its copy are not retrievable",
        )
    request(
        "delete-copy",
        "DELETE",
        f"/memories/{copy_memory_id}",
        204,
        route="/memories/{id}",
    )
...
def main() -> int:
    ...
    try:
        ...
        if not wait_health(f"{family}/api/health") or not wait_health(
            f"{admin}/admin-api/health"
        ):
...
def run_cases(suite: Suite, family: str, admin: str, data_dir: Path) -> None:
    ...
    record(
        suite, "family-refresh", "family", "POST", "/auth/refresh", resp, started, 200
    )
    ...
    space_id = next(
        (
            s.get("id")
            for s in spaces
            if isinstance(s, dict) and s.get("kind") == "household"
        ),
        None,
    )
    if space_id is None:
    ...
    started = time.monotonic()
    resp = client.get(
        f"{family}/api/personal-family-view",
        params={"space_id": space_id},
        headers=fam_headers,
    )
    record(
    ...
    started = time.monotonic()
    resp = client.get(
        f"{family}/api/stats", params={"space_id": space_id}, headers=fam_headers
    )
    ...
    started = time.monotonic()
    resp = client.post(
        f"{family}/api/notifications/read-all",
        json={"space_id": space_id},
        headers=fam_headers,
    )
    record(
        suite,
        "family-notifications-read-all",
        "family",
        "POST",
    ...
    started = time.monotonic()
    resp = client.get(
        f"{family}/api/memories", params={"space_id": space_id}, headers=fam_headers
    )
    ...
    started = time.monotonic()
    resp = client.post(
        f"{family}/api/agent/sessions", json={"space_id": space_id}, headers=fam_headers
    )
    record(
        suite,
    ...
    record(
        ...
        404,
        expect_code=None,
        note="SSE unknown run terminal state",
    )
    ...
    record(
        suite,
        "family-attachment-upload",
        "family",
        "POST",
        "/users/{id}/attachments/image",
        resp,
        started,
        201,
    )
    ...
    if attachment_id is not None:
        started = time.monotonic()
        resp = client.get(
            f"{family}/api/attachments/{attachment_id}/raw", headers=fam_headers
        )
        record(
            suite,
            "family-attachment-raw-authorized",
            "family",
            "GET",
            "/attachments/{id}/raw",
            resp,
        ...
        outsider_access = (
            outsider.json().get("access_token", "")
            if outsider.status_code in (200, 201)
            else ""
        )
        if outsider_access:
            resp = client.get(
            ...
            record(
                ...
                started,
                404,
                note="anti-enumeration safe failure for unrelated principal",
            )
        else:
            suite.add(
                Result(
                    ...
                    "GET",
                    "/attachments/{id}/raw",
                    note=f"outsider register unavailable ({outsider.status_code})",
                    passed=False,
                )
            )
        ...
        started = time.monotonic()
        resp = client.delete(
            f"{family}/api/attachments/{attachment_id}", headers=fam_headers
        )
        record(
            suite,
            "family-attachment-delete",
            "family",
            "DELETE",
            "/attachments/{id}",
            resp,
    ...
    if cred_path.exists():
        ...
        record(
            suite, "admin-me", "admin", "GET", "/admin-api/auth/me", resp, started, 200
        )
        ...
        started = time.monotonic()
        resp = client.get(
            f"{admin}/admin-api/v1/spaces/{space_id}", headers=adm_headers
        )
        record(
            suite,
            "admin-space-detail",
        ...
        record(
            suite,
            "admin-governance-write-probe",
            "admin",
            "POST",
            "/admin-api/v1/manager-applications/{id}/reject",
            resp,
            started,
            404,
            note="governance write endpoint mounted, unknown id -> uniform 404",
        )
    else:
        suite.add(
            Result(
                ...
                "POST",
                "/admin-api/auth/login",
                note="bootstrap credentials file missing",
                passed=False,
            )
        )
    ...
    started = time.monotonic()
    resp = client.get(
        f"{admin}/admin-api/v1/overview",
        headers={"Authorization": f"Bearer {family_access}"},
    )
    record(
        suite,
        "cross-family-token-on-admin",
        "admin",
        "GET",
        "/admin-api/v1/overview",
        resp,
        started,
        401,
        note="family token must not open admin listener",
    )
    ...
    if admin_access_for_cross:
        ...
        resp = client.get(
            f"{family}/api/me",
            headers={"Authorization": f"Bearer {admin_access_for_cross}"},
        )
        record(
            suite,
            "cross-admin-token-on-family",
            "family",
            "GET",
            "/me",
            resp,
            started,
            401,
            note="admin token must not open family listener",
        )
    else:
        suite.add(
            Result(
    ...
    suite.add(
        Result(
            ...
            ms=int((time.monotonic() - started) * 1000),
            note="byte-identical to unknown route 404"
            if same_shell
            else "404 shells differ",
            passed=same_shell,
        )
    )
    suite.add(
