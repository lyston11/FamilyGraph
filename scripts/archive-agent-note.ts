 ...
 * Usage: node --experimental-strip-types scripts/archive-agent-note.ts .agent-notes/implemented/<class>/<filename>.md
 */
...
if (!targetArg) {
  console.error("Usage: node --experimental-strip-types scripts/archive-agent-note.ts <path-to-note>");
  process.exit(1);
}
...
if (segs[0] !== "implemented" || segs.length !== 3) {
  console.error(`Error: only notes in .agent-notes/implemented/<class>/ can be archived (got: ${relToRoot})`);
  process.exit(1);
}
