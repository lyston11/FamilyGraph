 ...
 * Portable: resolves .agent-notes from cwd (or AGENT_NOTE_ROOT env).
 */
...
function resolveAgentNoteRoot(): string {
  ...
  // walk up from cwd to find .agent-notes, fallback to cwd/.agent-notes
  let cur = process.cwd()
  for (let i = 0; i < 6; i++) {
    const cand = resolve(cur, '.agent-notes')
    if (existsSync(cand)) return cand
    const parent = resolve(cur, '..')
    if (parent === cur) break
    cur = parent
  }
  return resolve(process.cwd(), '.agent-notes')
}

export const agentNoteRoot = resolveAgentNoteRoot()

// Note: 项目用路径即状态的 Agent Notes 取代 Trellis 任务树 — 见 .agent-notes/implemented/process/2026-09-11-adopt-agent-notes-and-deprecate-trellis.md
